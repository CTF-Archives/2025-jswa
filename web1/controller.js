    ...
    if(checkcode.length === 16){
        try{
            ...
            if(checkcode !== "aGr5AtSp55dRacer"){
                res.status(403).json({"msg":"Invalid Checkcode:" + checkcode})
            }
        }catch (__) {}
        res.status(200).type("text/html").json({"msg":"Flag2: " + flag2.toString().trim()})
    }else{
        res.status(403).type("text/html").json({"msg":"Invalid Checkcode:" + checkcode})
    }
}
...
function Flag1Controller(req,res){
    try {
        ...
        if(req.cookies.user === "admin") {
            res.setHeader("Token", flag1.toString().trim())
            res.status(200).type("text/html").send("This_Is_The_Flag")
        }else{
